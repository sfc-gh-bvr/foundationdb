...
public:
	...
	ACTOR Future<Void> fetchSourceServersForRange(Database cx,
	                                              RelocateData input,
	...
	DDQueue(UID mid,
	        ...
	        PromiseStream<GetMetricsRequest> getShardMetrics,
	        double* lastLimited)
	  : lock(lock), startMoveKeysParallelismLock(SERVER_KNOBS->DD_MOVE_KEYS_PARALLELISM),
	    finishMoveKeysParallelismLock(SERVER_KNOBS->DD_MOVE_KEYS_PARALLELISM),
	    fetchSourceLock(new FlowLock(SERVER_KNOBS->DD_FETCH_SOURCE_PARALLELISM)), teamSize(teamSize),
	    singleRegionTeamSize(singleRegionTeamSize), lastInterval(0), suppressIntervals(0),
	    movedKeyServersEventHolder(makeReference<EventCacheHolder>("MovedKeyServers")), cx(cx), distributorId(mid),
	    lastLimited(lastLimited), activeRelocations(0), queuedRelocations(0), unhealthyRelocations(0), bytesWritten(0),
	    input(input), output(output), teamCollections(teamCollections), getShardMetrics(getShardMetrics),
	    getAverageShardBytes(getAverageShardBytes), rawProcessingUnhealthy(new AsyncVar<bool>(false)),
	    rawProcessingWiggle(new AsyncVar<bool>(false)), shardsAffectedByTeamFailure(sABTF) {}
};
...
public:
	...
	ACTOR static Future<Void> dataDistributionRelocator(DDQueue* ddQueue,
	                                                    RelocateData rd,
	                                                    const DDEnabledState* ddEnabledState) {
		state Promise<Void> errorOut(ddQueue->error);
		state TraceInterval relocateShardInterval("RelocateShard");
		state PromiseStream<RelocateData> dataTransferComplete(ddQueue->dataTransferComplete);
		state PromiseStream<RelocateData> relocationComplete(ddQueue->relocationComplete);
		state bool signalledTransferComplete = false;
		state UID distributorId = ddQueue->distributorId;
		...
		try {
			if (now() - ddQueue->lastInterval < 1.0) {
				relocateShardInterval.severity = SevDebug;
				ddQueue->suppressIntervals++;
			}
			...
			TraceEvent(relocateShardInterval.begin(), distributorId)
			    ...
			    .detail("SuppressedEventCount", ddQueue->suppressIntervals);

			if (relocateShardInterval.severity != SevDebug) {
				ddQueue->lastInterval = now();
				ddQueue->suppressIntervals = 0;
			}

			state StorageMetrics metrics =
			    wait(brokenPromiseToNever(ddQueue->getShardMetrics.getReply(GetMetricsRequest(rd.keys))));
			...
			loop {
				...
				loop {
					...
					while (tciIndex < ddQueue->teamCollections.size()) {
						...
						Future<std::pair<Optional<Reference<IDataDistributionTeam>>, bool>> fbestTeam =
						    brokenPromiseToNever(ddQueue->teamCollections[tciIndex].getTeam.getReply(req));
					...
					// already
					anyDestOverloaded = !canLaunchDest(bestTeams, rd.priority, ddQueue->destBusymap);
					...
					if (anyDestOverloaded) {
						...
						TraceEvent(destOverloadedCount > 50 ? SevInfo : SevDebug, "DestSSBusy", distributorId)
						    ...
						    .detail("NumOfTeamCollections", ddQueue->teamCollections.size())
				...
				auto inFlightRange = ddQueue->inFlight.rangeContaining(rd.keys.begin);
				...
				if (totalIds != ddQueue->teamSize) {
					TraceEvent(SevWarn, "IncorrectDestTeamSize")
					    .suppressFor(1.0)
					    .detail("ExpectedTeamSize", ddQueue->teamSize)
					    .detail("DestTeamSize", totalIds);
				}

				ddQueue->shardsAffectedByTeamFailure->moveShard(rd.keys, destinationTeams);

				// FIXME: do not add data in flight to servers that were already in the src.
				healthyDestinations.addDataInFlightToTeam(+metrics.bytes);

				launchDest(rd, bestTeams, ddQueue->destBusymap);
				...
				state Future<Void> doMoveKeys = moveKeys(ddQueue->cx,
				                                         rd.keys,
				                                         destIds,
				                                         healthyIds,
				                                         ddQueue->lock,
				                                         dataMovementComplete,
				                                         &ddQueue->startMoveKeysParallelismLock,
				                                         &ddQueue->finishMoveKeysParallelismLock,
				                                         ddQueue->teamCollections.size() > 1,
				                                         relocateShardInterval.pairID,
				                                         ddEnabledState);
				state Future<Void> pollHealth = signalledTransferComplete ? Never()
						...
						choose {
							when(wait(doMoveKeys)) {
								if (extraIds.size()) {
									...
									doMoveKeys = moveKeys(ddQueue->cx,
									                      rd.keys,
									                      destIds,
									                      healthyIds,
									                      ddQueue->lock,
									...
									ddQueue->fetchKeysComplete.insert(rd);
									break;
								}
							}
							when(wait(pollHealth)) {
								if (!healthyDestinations.isHealthy()) {
									if (!signalledTransferComplete) {
										...
										ddQueue->dataTransferComplete.send(rd);
									}
								}
					...
					if (!error.code()) {
						...
						if (rd.keys.begin == keyServersPrefix) {
							TraceEvent("MovedKeyServerKeys")
							    .detail("Dest", describe(destIds))
							    .trackLatest(ddQueue->movedKeyServersEventHolder->trackingKey);
						}
						...
						ddQueue->bytesWritten += metrics.bytes;
						ddQueue->shardsAffectedByTeamFailure->finishMove(rd.keys);
...
ACTOR Future<Void> dataDistributionQueue(Database cx,
                                         ...
                                         double* lastLimited,
                                         const DDEnabledState* ddEnabledState) {
	state DDQueue ddQueue(distributorId,
	                      lock,
	                      cx,
	                      teamCollections,
	...
	for (int i = 0; i < teamCollections.size(); i++) {
		balancingFutures.push_back(BgDDMountainChopper(&ddQueue, i));
		balancingFutures.push_back(BgDDValleyFiller(&ddQueue, i));
	}
	balancingFutures.push_back(delayedAsyncVar(ddQueue.rawProcessingUnhealthy, processingUnhealthy, 0));
	balancingFutures.push_back(delayedAsyncVar(ddQueue.rawProcessingWiggle, processingWiggle, 0));
	...
	try {
		loop {
			...
			if (launchData.startTime != -1) {
				...
				ddQueue.launchQueuedWork(launchData, ddEnabledState);
				launchData = RelocateData();
			} else if (!keysToLaunchFrom.empty()) {
				ddQueue.launchQueuedWork(keysToLaunchFrom, ddEnabledState);
			...
			choose {
				when(RelocateShard rs = waitNext(ddQueue.input)) {
					bool wasEmpty = serversToLaunchFrom.empty();
					ddQueue.queueRelocation(rs, serversToLaunchFrom);
				...
				when(wait(launchQueuedWorkTimeout)) {
					ddQueue.launchQueuedWork(serversToLaunchFrom, ddEnabledState);
					serversToLaunchFrom = std::set<UID>();
					launchQueuedWorkTimeout = Never();
				}
				when(RelocateData results = waitNext(ddQueue.fetchSourceServersComplete.getFuture())) {
					// This when is triggered by queueRelocation() which is triggered by sending self.input
					ddQueue.completeSourceFetch(results);
					launchData = results;
				}
				when(RelocateData done = waitNext(ddQueue.dataTransferComplete.getFuture())) {
					complete(done, ddQueue.busymap, ddQueue.destBusymap);
					if (serversToLaunchFrom.empty() && !done.src.empty())
						launchQueuedWorkTimeout = delay(0, TaskPriority::DataDistributionLaunch);
					serversToLaunchFrom.insert(done.src.begin(), done.src.end());
				}
				when(RelocateData done = waitNext(ddQueue.relocationComplete.getFuture())) {
					ddQueue.activeRelocations--;
					ddQueue.finishRelocation(done.priority, done.healthPriority);
					ddQueue.fetchKeysComplete.erase(done);
				...
				when(wait(recordMetrics)) {
					...
					for (auto it = ddQueue.priority_relocations.begin(); it != ddQueue.priority_relocations.end();
					     ++it) {
						if (it->second) {
					...
					TraceEvent("MovingData", distributorId)
					    .detail("InFlight", ddQueue.activeRelocations)
					    .detail("InQueue", ddQueue.queuedRelocations)
					    .detail("AverageShardSize", req.getFuture().isReady() ? req.getFuture().get() : -1)
					    .detail("UnhealthyRelocations", ddQueue.unhealthyRelocations)
					    .detail("HighestPriority", highestPriorityRelocation)
					    .detail("BytesWritten", ddQueue.bytesWritten)
					    .detail("PriorityRecoverMove",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_RECOVER_MOVE])
					    .detail("PriorityRebalanceUnderutilizedTeam",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_REBALANCE_UNDERUTILIZED_TEAM])
					    .detail("PriorityRebalanceOverutilizedTeam",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_REBALANCE_OVERUTILIZED_TEAM])
					    .detail("PriorityStorageWiggle",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_PERPETUAL_STORAGE_WIGGLE])
					    .detail("PriorityTeamHealthy",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_TEAM_HEALTHY])
					    .detail("PriorityTeamContainsUndesiredServer",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_TEAM_CONTAINS_UNDESIRED_SERVER])
					    .detail("PriorityTeamRedundant",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_TEAM_REDUNDANT])
					    .detail("PriorityMergeShard", ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_MERGE_SHARD])
					    .detail("PriorityPopulateRegion",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_POPULATE_REGION])
					    .detail("PriorityTeamUnhealthy",
					            ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_TEAM_UNHEALTHY])
					    .detail("PriorityTeam2Left", ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_TEAM_2_LEFT])
					    .detail("PriorityTeam1Left", ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_TEAM_1_LEFT])
					    .detail("PriorityTeam0Left", ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_TEAM_0_LEFT])
					    .detail("PrioritySplitShard", ddQueue.priority_relocations[SERVER_KNOBS->PRIORITY_SPLIT_SHARD])
				...
				when(wait(ddQueue.error.getFuture())) {} // Propagate errors from dataDistributionRelocator
				when(wait(waitForAll(balancingFutures))) {}
				when(Promise<int> r = waitNext(getUnhealthyRelocationCount.getFuture())) {
					r.send(ddQueue.unhealthyRelocations);
				}
			}
		}
	} catch (Error& e) {
...
ACTOR Future<Void> DDQueue::fetchSourceServersForRange(Database cx,
                                                       RelocateData input,
                                                       PromiseStream<RelocateData> output,
                                                       Reference<FlowLock> fetchLock) {
	...
	for (int r = 0; r < affectedQueuedItems.size(); ++r, ++queueMapItr) {
		...
		if (rrs.src.size() == 0 && (rrs.keys == rd.keys || fetchingSourcesQueue.erase(rrs) > 0)) {
			...
			getSourceActors.insert(rrs.keys,
			                       fetchSourceServersForRange(cx, rrs, fetchSourceServersComplete, fetchSourceLock));
		} else {
...
void DDQueue::launchQueuedWork(std::set<RelocateData, std::greater<RelocateData>> combined,
	...
	for (; it != combined.end(); it++) {
		...
		for (int r = 0; r < ranges.size(); r++) {
			...
			inFlightActors.insert(rrs.keys, DDQueueImpl::dataDistributionRelocator(this, rrs, ddEnabledState));
		}
